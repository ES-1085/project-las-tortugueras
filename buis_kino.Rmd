...
```{r GTBK-static-turtle-growth, fig.width = 10, fig.height = 6}
...
GTBK %>%
  ...
  ggplot(aes(x = year, y = l_c_c, size = l_c_c, color = marca_nombre)) +
...
GTBK %>%
  ...
  geom_point(aes(group = year), size = 4, alpha = 0.7, show.legend = FALSE) +
    scale_color_manual(values = c("indianred3", "goldenrod", "palegreen4")) +
    facet_wrap(~ marca_nombre) +
  coord_fixed(ratio = 0.3) +
  theme_bw() +
  labs(title = "Growth of 3 Individual Female Green Sea Turtles in Laguna la Cruz, Mexico",
