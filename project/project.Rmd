...
# This chunk isn't running. It is saying that most of the variables aren't found.  
GTBK_data <- GTBK_data %>% 
  subset(select = 
...
```{r recaptures-per-month,  fig.alt = "This is a bar chart representing the seasonality of overall sea turtle recaptures Each bar represents the total number of recaptures per outing per month. There is a roughly even seasonal distribution of recaptures.", echo=FALSE}
GTBK_data <- GTBK_data %>%
...
```{r individual_recaps_sankey, fig.alt = "This sankey diagram tracks the 9 most frequently recaptured over time from 2014 to 2021. Each turtle is labled with it's tag number. Generally it seems that individual's recaptures are clustered together, but some are more spread apart", echo=FALSE}
...
```{r captures_per_effort_over_time,  fig.alt = "This bar chart demonstrates the number of turtle recaptures per outing from 2010 to 2023. Recaptures per effort have stayed roughly the same through time.", echo=FALSE}
library(lubridate)
...
ggsave("final_visualizations/final_heat_map.jpg", device = "jpg", dpi = 500)
...
ggplot(aes(x = año, fill = geografía)) +
  ...
  scale_fill_manual(values = c("cyan4", "cyan2", "coral1")) +
   theme_bw()
ggsave("final_visualizations/final_spacial_overview.jpg", device = "jpg", dpi = 500)
```
...
scale_fill_manual(values = c("gold", "darkseagreen3", "mediumorchid", "darkolivegreen")) +
   theme_bw()
ggsave("final_visualizations/final_spacial_species_breakdown.jpg", device = "jpg", dpi = 500)
