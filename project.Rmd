...
GTBK_used_variables <- 
  GTBK_data %>%
   select(marca_nombre, fecha, area_de_monitoreo, especie, sexo, l_r_c, l_c_c, a_r_c, a_c_c, p_c, l_p, l_t_c)

visdat::vis_miss(GTBK_used_variables)
```
...
```{r turtles_per_effort_over_time}
...
```{r Turtles per effort over time}
library(lubridate)
...
# wanted to add this so that the graph below would not have to be continuous, more defined

tortugas_por_esfuerzo <- tortugas_por_esfuerzo %>%
  mutate(mes_nombre =
           case_when(
             mes == "1" ~ "jan",
             mes == "2" ~ "feb",
...
tortugas_por_esfuerzo %>%
  group_by(año, mes) %>%
   ggplot(aes(x = año, y = esfuerzo))+
   geom_point()


#scale_fill_gradient(low = "coral1", high = "dodgerblue4")
...
#I wish this wasnt continuous so moths could be more distinct
...
lugar_de_tortugas <- GTBK_data %>%
  mutate(mes = month(fecha)) %>%
  mutate(año = year(fecha)) %>%
  select(marca_nombre, lugar, geografía, numero_recaptura, mes, año)

ggplot(data = lugar_de_tortugas, 
       mapping = aes(x = año, fill = geografía)) +
  geom_bar() +
